...
class CoreEngineGui(ctk.CTk):
    ...
    def setup_top_dashboard(self):
        ...
        # Fast Inline Album Creator Layout
        ...
        delete_album_btn = ctk.CTkButton(self.dashboard, text="🗑️ Delete Album", width=110, fg_color="#d9534f", hover_color="#c9302c", command=self.ui_delete_album)
        delete_album_btn.pack(side="right", padx=5, pady=20)
    ...
    def refresh_album_dropdown(self):
        ...
        # FIX: Explicit 0 handles compilation hook tracer requirements safely
    ...
    def ui_delete_album(self):
        """Triggers the pure C backend layer to wipe target files from drive tracks"""
        if not self.current_album or self.current_album == "No Albums Found":
            return
